...
import { postBookingData } from '@/lib/action/action'
 ...
 function GroundDetailsPage({ groundDetails }) {
    ...
    const totalPrice = price_per_hour * (Number(duration) || 0)
    ...
    if (departureDate) {
        ...
        const month = String(departureDate.month).padStart(2, '0');
    ...
    try {
        const result = await postBookingData(bookingData);
                ...
                <div className="bg-slate-950/50 lg:border-l border-slate-800 p-6 md:p-8 flex flex-col justify-between">
                    <Form className="flex flex-col gap-5 w-full">
                        ...
                        <TextField isRequired name="date" type="date" className="flex flex-col gap-1.5">
                            ...
                            <DateFieldBar departureDate={departureDate} setDepartureDate={setDepartureDate} />
                            <FieldError className="text-xs text-red-400 mt-1" />
                        </TextField>
                        ...
                        <button onClick={handleBooking} type="submit" className="w-full bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 text-slate-950 font-bold tracking-wide h-12 text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/10 hover:shadow-emerald-500/20 mt-2 btn">
                            Continue Booking
                        </button>
                    </Form>
                </div>
